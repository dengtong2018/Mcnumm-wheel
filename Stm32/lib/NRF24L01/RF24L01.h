...
#define RF24L01_SPI SPI1

/** RF24L01硬件接口定义 */
#define RF24L01_CE_GPIO_PORT			GPIOD                      
#define RF24L01_CE_CLK_ENB			       __HAL_RCC_GPIOD_CLK_ENABLE()
#define RF24L01_CE_GPIO_PIN			GPIO_PIN_9
#define RF24L01_CE_HIGH()				PDSET(RF24L01_CE_GPIO_PIN)
#define RF24L01_CE_LOW()				PDRESET(RF24L01_CE_GPIO_PIN)

#define RF24L01_IRQ_GPIO_PORT			GPIOD                         
#define RF24L01_IRQ_CLK_ENB			__HAL_RCC_GPIOD_CLK_ENABLE()
#define RF24L01_IRQ_GPIO_PIN			GPIO_PIN_8                          
#define RF24L01_IRQ_GET_STATUS( )		HAL_GPIO_ReadPin(RF24L01_IRQ_GPIO_PORT , RF24L01_IRQ_GPIO_PIN)	//IRQ状态


#define RF24L01_CS_GPIO_PORT			SPI_NSS_GPIO_PORT
#define RF24L01_CS_GPIO_CLK			SPI_NSS_CLK_ENB
#define RF24L01_CS_GPIO_PIN			SPI_NSS_GPIO_PIN
#define RF24L01_CS_HIGH()				SPI_NSS_HIGH
#define RF24L01_CS_LOW()				SPI_NSS_LOW
...
class NRF24L01
{
	public:
		...
		void Read_Buf( uint8_t RegAddr, uint8_t *pBuf, uint8_t len );
		void Write_Reg( uint8_t RegAddr, uint8_t Value );
		void Write_Buf( uint8_t RegAddr, uint8_t *pBuf, uint8_t len );
		...
		uint8_t Read_IRQ_Status( void );
		uint8_t Read_Top_Fifo_Width( void );
		uint8_t Read_Rx_Payload( uint8_t *pRxBuf );
		void Write_Tx_Payload_Ack( uint8_t *pTxBuf, uint8_t len );
		void Write_Tx_Payload_NoAck( uint8_t *pTxBuf, uint8_t len );
		void Write_Tx_Payload_InAck( uint8_t *pData, uint8_t len );
		void Set_TxAddr( uint8_t *pAddr, uint8_t len );
		void Set_RxAddr( uint8_t PipeNum, uint8_t *pAddr, uint8_t Len );
		void Set_Speed( nRf24l01SpeedType Speed );
		void Set_Power( nRf24l01PowerType Power );
		void Write_Hopping_Point( uint8_t FreqPoint );
		void Set_Mode( nRf24l01ModeType Mode );
		void  check( void );
