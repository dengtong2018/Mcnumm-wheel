...
uint8_t NRF24L01::Read_Reg( uint8_t RegAddr )
{
    ...
    Spi1SendAndRecvByte( NRF_READ_REG | RegAddr );	//读命令 地址
    btmp = Spi1SendAndRecvByte( 0xFF );				//读数据
	
    RF24L01_CS_HIGH( );			//取消片选
...
void NRF24L01::Read_Buf( uint8_t RegAddr, uint8_t *pBuf, uint8_t len )
{
    ...
    {
        *( pBuf + btmp ) = Spi1SendAndRecvByte( 0xFF );	//读数据
    }
    RF24L01_CS_HIGH( );		//取消片选
}
...
void NRF24L01::Write_Reg( uint8_t RegAddr, uint8_t Value )
{
    ...
    Spi1SendAndRecvByte( NRF_WRITE_REG | RegAddr );	//写命令 地址
    Spi1SendAndRecvByte( Value );			//写数据
...
void NRF24L01::Write_Buf( uint8_t RegAddr, uint8_t *pBuf, uint8_t len )
{
    ...
    for( i = 0; i < len; i ++ )
    {
        Spi1SendAndRecvByte( *( pBuf + i ) );		//写数据
    }
...
void NRF24L01::Flush_Tx_Fifo ( void )
{
    ...
    Spi1SendAndRecvByte( FLUSH_TX );	//清TX FIFO命令
...
void NRF24L01::Flush_Rx_Fifo( void )
{
    ...
    Spi1SendAndRecvByte( FLUSH_RX );	//清RX FIFO命令
...
void NRF24L01::Reuse_Tx_Payload( void )
{
    ...
    Spi1SendAndRecvByte( REUSE_TX_PL );		//重新使用上一包命令
...
uint8_t NRF24L01::Read_Status_Register( void )
{
    ...
    Status = Spi1SendAndRecvByte( NRF_READ_REG + STATUS );	//读状态寄存器
...
uint8_t NRF24L01::Clear_IRQ_Flag( uint8_t IRQ_Source )
{
    ...
    btmp = Read_Status_Register( );			//读状态寄存器
			
    RF24L01_CS_LOW( );			//片选
    Spi1SendAndRecvByte( NRF_WRITE_REG + STATUS );	//写状态寄存器命令
    Spi1SendAndRecvByte( IRQ_Source | btmp );		//清相应中断标志
    RF24L01_CS_HIGH( );			//取消片选
	
    return ( Read_Status_Register( ));			//返回状态寄存器状态
}
...
{
    return ( Read_Status_Register( ) & (( 1 << RX_DR ) | ( 1 << TX_DS ) | ( 1 << MAX_RT )));	//返回中断状态
}
...
uint8_t NRF24L01::Read_Top_Fifo_Width( void )
{
    ...
    Spi1SendAndRecvByte( R_RX_PL_WID );	//读FIFO中数据宽度命令
    btmp = Spi1SendAndRecvByte( 0xFF );	//读数据
...
uint8_t NRF24L01::Read_Rx_Payload( uint8_t *pRxBuf )
{
    ...
    PipeNum = ( Read_Reg( STATUS ) >> 1 ) & 0x07;	//读接收状态
    Width = Read_Top_Fifo_Width( );		//读接收数据个数

    RF24L01_CS_LOW( );		//片选
    Spi1SendAndRecvByte( RD_RX_PLOAD );			//读有效数据命令
	
    for( PipeNum = 0; PipeNum < Width; PipeNum ++ )
    {
        *( pRxBuf + PipeNum ) = Spi1SendAndRecvByte( 0xFF );		//读数据
    }
    RF24L01_CS_HIGH( );		//取消片选
    Flush_Rx_Fifo( );	//清空RX FIFO
...
void NRF24L01::Write_Tx_Payload_Ack( uint8_t *pTxBuf, uint8_t len )
{
    ...
    Flush_Tx_Fifo( );		//清TX FIFO
	
    RF24L01_CS_LOW( );			//片选
    Spi1SendAndRecvByte( WR_TX_PLOAD );	//发送命令
	
    for( btmp = 0; btmp < length; btmp ++ )
    {
        Spi1SendAndRecvByte( *( pTxBuf + btmp ) );	//发送数据
    }
    RF24L01_CS_HIGH( );			//取消片选
}
...
void NRF24L01::Write_Tx_Payload_NoAck( uint8_t *pTxBuf, uint8_t len )
{
    ...
    RF24L01_CS_LOW( );	//片选
    Spi1SendAndRecvByte( WR_TX_PLOAD_NACK );	//发送命令
    while( len-- )
    {
        Spi1SendAndRecvByte( *pTxBuf );			//发送数据
		pTxBuf++;
    }
    RF24L01_CS_HIGH( );		//取消片选
}
...
void NRF24L01::Write_Tx_Payload_InAck( uint8_t *pData, uint8_t len )
{
    ...
    RF24L01_CS_LOW( );			//片选
    Spi1SendAndRecvByte( W_ACK_PLOAD );		//命令
    for( btmp = 0; btmp < len; btmp ++ )
    {
        Spi1SendAndRecvByte( *( pData + btmp ) );	//写数据
    }
    RF24L01_CS_HIGH( );			//取消片选
}
...
void NRF24L01::Set_TxAddr( uint8_t *pAddr, uint8_t len )
{
	len = ( len > 5 ) ? 5 : len;					//地址不能大于5个字节
    	Write_Buf( TX_ADDR, pAddr, len );	//写地址
}
...
void NRF24L01::Set_RxAddr( uint8_t PipeNum, uint8_t *pAddr, uint8_t Len )
{
    ...
    Write_Buf( RX_ADDR_P0 + PipeNum, pAddr, Len );	//写入地址
}
...
void NRF24L01::check( void )
{
	...
	while( 1 )
	{
		Write_Buf( TX_ADDR, buf, 5 );			//写入5个字节的地址
		Read_Buf( TX_ADDR, read_buf, 5 );		//读出写入的地址  
		for( i = 0; i < 5; i++ )
		{
			if( buf[ i ] != read_buf[ i ] )
			{
...
uint8_t NRF24L01::TxPacket( uint8_t *txbuf, uint8_t Length )
{
	...
	RF24L01_CE_LOW( );		
	Write_Buf( WR_TX_PLOAD, txbuf, Length );	//写数据到TX BUF 32字节  TX_PLOAD_WIDTH
	RF24L01_CE_HIGH( );			//启动发送
	...
	Write_Reg( STATUS, l_Status );						//清除TX_DS或MAX_RT中断标志
...
uint8_t NRF24L01::RxPacket( uint8_t *rxbuf )
{
	...
	if( l_Status & RX_OK)	//接收到数据
	{
		...
		Read_Buf( RD_RX_PLOAD,rxbuf,l_RxLength );	//接收到数据 
		Write_Reg( FLUSH_RX,0xff );				//清除RX FIFO
		return l_RxLength; 
	}	
...
void NRF24L01::Init( void )
{
	...
	Write_Reg( DYNPD, ( 1 << 0 ) ); 	//使能通道1动态数据长度
	Write_Reg( FEATRUE, 0x07 );
	Read_Reg( DYNPD );
	Read_Reg( FEATRUE );

	Write_Reg( CONFIG, /*( 1<<MASK_RX_DR ) |*/		//接收中断
	                          ( 1 << EN_CRC ) |     //使能CRC 1个字节
	                          ( 1 << PWR_UP ) );    //开启设备
	Write_Reg( EN_AA, ( 1 << ENAA_P0 ) );   		//通道0自动应答
	Write_Reg( EN_RXADDR, ( 1 << ERX_P0 ) );		//通道0接收
	Write_Reg( SETUP_AW, AW_5BYTES );     			//地址宽度 5个字节
	Write_Reg( SETUP_RETR, ARD_4000US |
	            ( REPEAT_CNT & 0x0F ) );         	//重复等待时间 250us
	Write_Reg( RF_CH, 60 );             			//初始化通道
	Write_Reg( RF_SETUP, 0x26 );

	Set_TxAddr( &addr[0], 5 );                      //设置TX地址
	Set_RxAddr( 0, &addr[0], 5 );                   //设置RX地址
